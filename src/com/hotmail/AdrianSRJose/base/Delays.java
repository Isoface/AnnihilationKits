...
public class Delays {
	...
	private Delays() {
		...
		Bukkit.getScheduler().scheduleSyncRepeatingTask(AnnihilationMain.INSTANCE, new DelayUpdater(this), 20L, 20L);
	}
	...
	public void removeDelay(String delayName, DelayUpdate handler) {
		if (!delayIDs.containsKey(delayName.toLowerCase())) {
			delayIDs.remove(delayName.toLowerCase(), counter);
			delayHandlers.remove(counter, handler);
			counter++;
		}
	}
	...
	public boolean hasActiveDelay(Player p, String delayType) {
		for (Delay d : delays) {
			if (d.playerID.equals(p.getUniqueId()) && d.DelayID == delayIDs.get(delayType.toLowerCase())
					&& System.currentTimeMillis() < d.endTime)
				return true;
		}
		return false;
	}

	public Delay addDelay(Player p, long endTime, String delay) {
	...
	public void removeDelay(Player p, long endTime, String delay) {
		Integer id = delayIDs.get(delay.toLowerCase());
		Delay d = new Delay(p.getUniqueId(), id, endTime, delayHandlers.get(id));
		delays.remove(d);
	}
